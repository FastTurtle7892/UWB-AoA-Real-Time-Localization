...
void change_rf_left_first();
void change_rf_right_first();

double correctionPdoa(double pdoa,int channel);
double calculateAoA_1(double pdoa, int channel);
double calculateAoA_2(double pdoa, int channel);
...
uint32_t AoA_array[4];
#define MAX_VALUES 200
#define MAX_STRING_LENGTH 1024
static uint32_t rf_switch_status = 0;
int aoa_rtls_responder_two(void)
{

    //change_rf_left_first();
    //change_rf_right_first();

    //rf_switch_status = dwt_read32bitreg(RF_SWITCH_CTRL_ID);
    //printf("%d\n", rf_switch_status);

    // PART1. INITALIZE 
    int sensorIndex = 1;
    int fileIndex = 0;
    char sensorData[MAX_VALUES * MAX_STRING_LENGTH]; 
    sensorData[0] = '\0';
    ...
    while (1)
    {
        ...
        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
        {
            ...
            if(dwt_readstsquality(&stsqual1)) //if STS is good this will be true >= 0
            {
...
//----------------------------------------------------poll _ AoA -----------------------------------------------------------------------
                        ...
                        pdoa1 = dwt_readpdoa();
                        pdoa_deg1 = ((float)pdoa1 / (1 << 11));
    
                        
                        pdoa_deg1 = correctionPdoa(pdoa_deg1,5); // PDoA calibration 
                        //printf("pdoa_deg_1:%f\n",pdoa_deg1);
                        prev = AoA1;
                        AoA1 = calculateAoA_1(pdoa_deg1,5);
                        //printf("AoA_1 :%g\n",AoA1);

                         dwt_otpread(0x000c, AoA_array, 4);
                         printf("Poll AoA1 : %d | Poll AoA2 : %d\n", AoA_array[0],AoA_array[1]);
...
//--------------------------------------------------------------------------------------------------------------------------------------
                        ...
                        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
                        {
                            ...
                            //as STS mode is used, we only consider frames that are received with good STS quality
                            if(dwt_readstsquality(&stsqual2))  //if STS is good this will be true >= 0
                            {
...
//-----------------------------------------------------------------final _AoA-----------------------------------------------------------
                                    ...
                                    dwt_readdiagnostics(&rx_diag);
                                    pdoa2 = dwt_readpdoa();
                                    pdoa_deg2 = ((float)pdoa2 / (1 << 11));
                
                                    
                                    pdoa_deg2 = correctionPdoa(pdoa_deg2,5); // PDoA calibration 
                                    //printf("pdoa_deg2:%f\n",pdoa_deg2);
                                    prev = AoA2;
                                    AoA2 = calculateAoA_2(pdoa_deg2,5);
                                    //printf("AoA2 :%g\n",AoA2);

                                    dwt_otpread(0x000c, AoA_array, 4);
                                    printf("Final AoA1 : %d | Final AoA2 : %d\n", AoA_array[0], AoA_array[1]);
...
//-----------------------------------------------file storage--------------------------------------------------------------------
                                    ...
                                    char tempString_dist[MAX_STRING_LENGTH];
                                    char tempString_angle[MAX_STRING_LENGTH];
                                    char tempString_stsqual[MAX_STRING_LENGTH];
      
                                    snprintf(tempString_stsqual, sizeof(tempString_stsqual), "%d|%d|", stsqual1, stsqual2);
                                    snprintf(tempString_dist, sizeof(tempString_dist), "%f|", distance);
                                    snprintf(tempString_angle, sizeof(tempString_angle), "%f|%f\n", pdoa_deg1, pdoa_deg2);
                                      
                                    strcat(sensorData, tempString_stsqual);
                                    strcat(sensorData, tempString_dist);
                                    strcat(sensorData, tempString_angle); 
              
                                    sensorIndex++;

                                    
                                    
                                    if(sensorIndex==MAX_VALUES){ 
                                        count_2++;
                                        char filename[256];
                                        //sprintf(filename, "h_006_030_switching.txt");
                                        
                                        //sprintf(filename, "h_60cm_150_left.txt");
                                        sprintf(filename, "h_60cm_150_right.txt"); 
...
double calculateAoA_1(double pdoa,int channel){
      ...
      if(channel == 5)
         AoA1 =(asin(pdoa*0.4321))*180/3.141592;
      else if(channel == 9)
         AoA1 = (asin(pdoa*0.3516))*180/3.141592;
...
double calculateAoA_2(double pdoa,int channel){
      ...
      if(channel == 5)
         AoA2 =(asin(pdoa*0.4321))*180/3.141592;
      else if(channel == 9)
         AoA2 = (asin(pdoa*0.3516))*180/3.141592;
...
uint32_t TIMER4_Check(uint32_t starttime, float clk_time){
        ...
        printf("elapsed_time = %f us\n\n", ticks * clk_time);
        return end_time;
}

void change_rf_left_first()
{
    dwt_write32bitreg(RF_ENABLE_ID, 0x02003c00);
    dwt_write32bitreg(RF_CTRL_MASK_ID, 0x02003c00);
    dwt_write32bitreg(RF_SWITCH_CTRL_ID, 0x00000000);
}
...
void change_rf_right_first()
{
    dwt_write32bitreg(RF_ENABLE_ID, 0x02003c00);
    dwt_write32bitreg(RF_CTRL_MASK_ID, 0x02003c00);
    dwt_write32bitreg(RF_SWITCH_CTRL_ID, 0x00000002);
}
