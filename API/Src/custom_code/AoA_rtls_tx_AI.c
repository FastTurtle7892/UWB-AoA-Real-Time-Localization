...
#if defined(AoA_RTLS_TX_AI)

extern void test_run_info(unsigned char *data);

/* Example application name and version to display on LCD screen. */
#define APP_NAME "AOA RTLS TX v1.0"
...
static uint8_t rx_ack_msg[]= { 0x41, 0x88, 0, 0xAA, 0xDE, 'V', 'E', 'W', 'A', 0x12, 0x02, 0, 0,0,0};
...
#define POLL_TX_TO_RESP_RX_DLY_UUS 770 // orignal value is 700
...
#define RESP_RX_TIMEOUT_UUS 1510 // orginal value is 1510.
...
int aoa_rtls_initiator_ai(void)
{   

    // PART1. Setting
      int sensorIndex = 1;
    int fileIndex = 0;
    char sensorData[MAX_VALUES * MAX_STRING_LENGTH]; // ���� �����͸� ������ ū ���ڿ�
    sensorData[0] = '\0'; 
    ...
    while (1)
    {
        ...
        {   
            printf("poll trans ok | resp receive");
            uint32_t frame_len;
            int16_t stsqual;
        ...
        uint32_t delay_time;
        delay_time = (final_tx_ts + (2500 * UUS_TO_DWT_TIME)) >> 8; // orignal value = 2500
           ...
           // printf("\nreceive distance from anchor A");
            ...
            if ((memcmp(rx_buffer, rx_ack_msg, 2) == 0) & (memcmp(rx_buffer + 5, rx_ack_msg + 5, 5) == 0)) {
               // printf("\nPANID %x", rx_buffer[3]);
                ...
                AoA = (bytesAsValue(&rx_buffer[ACK_MSG_AOA_IDX],2));
                printf("\nDIST: %3.2f | ", distance);
                printf("\AoA: %3.2f \n", AoA);
                char tempString[MAX_STRING_LENGTH];
                sprintf(tempString, "%f|%f\n", distance, AoA); // �������� ���ڿ��� ��ȯ

                strcat(sensorData, tempString); // ū ���ڿ��� �߰�
                
                sensorIndex++;
                //fprintf(file, "%g|%f\n", distance, AoA);
                if(sensorIndex==MAX_VALUES){ 
                    char filename[256];
                    sprintf(filename, "%d.txt", fileIndex); // ���ϸ� ����
                    //sprintf(filename, "/home/samsung/Desktop/SLAM_UWB_localization/Data_from_UWB/%d.txt", fileIndex);
                    fileIndex++;
                    ...
                    fprintf(file, "%s", sensorData); // ���ڿ� ��ü�� ���Ͽ� ����
